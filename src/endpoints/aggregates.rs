...
fn spool(data: &Arc<DashMap<i64, MinuteData>>,
         ...
         category_id: u16,
         process_origin: bool,
         process_brand_id: bool,
         process_category_id: bool) -> Vec<SpoolingResult> {
	...
	let result = par_itter.map(|minute| {
		...
		for i in 0..minute_data.product_id.len() {
			let correct_origin = !process_origin || (process_origin && minute_data.product_id[i] == origin);
			let correct_brand_id = !process_brand_id || (process_brand_id && minute_data.brand_id[i] == brand_id);
			let correct_category_id = !process_category_id || (process_category_id && minute_data.category_id[i] == category_id);
 ...
 */
#[post("/aggregates")]
pub async fn aggregates(_data: web::Data<AppState>, req_body: String, _info: web::Query<AggregateRequest>, _req: HttpRequest) -> impl Responder {
